...
@submenu_router.get(SUBMENUS_LINK,
                    summary='Все подменю конкретного меню',
...
@submenu_router.post(SUBMENUS_LINK,
                     summary='Добавить подменю',
                     status_code=status.HTTP_201_CREATED,
                     response_model=SubmenuRead,
                     responses={**MENU_NOT_FOUND, **VALUE_IS_EXIST})
async def post_submenu(menu_id: str,
                       submenu: SubmenuCreate,
                       service: SubmenuService = Depends()) -> dict[str, str | int]:
...
@submenu_router.get(SUBMENU_LINK,
                    summary='Получить подменю',
                    status_code=status.HTTP_200_OK,
                    response_model=SubmenuRead,
                    responses=SUBMENU_NOT_FOUND)
async def get_submenu(menu_id: str,
                      submenu_id: str,
                      service: SubmenuService = Depends()) -> dict[str, str | int]:
...
@submenu_router.patch(SUBMENU_LINK,
                      summary='Обновить подменю',
                      status_code=status.HTTP_200_OK,
                      response_model=SubmenuRead,
                      response_model_exclude_none=True,
                      responses={**SUBMENU_NOT_FOUND, **VALUE_IS_EXIST})
async def update_submenu(menu_id: str,
                         submenu_id: str,
                         updated_submenu: SubmenuUpdate,
                         service: SubmenuService = Depends()) -> dict[str, str]:
...
@submenu_router.delete(SUBMENU_LINK,
                       summary='Удалить подменю',
                       status_code=status.HTTP_200_OK,
                       responses=SUBMENU_NOT_FOUND)
async def delete_submenu(menu_id: str,
                         submenu_id: str,
                         service: SubmenuService = Depends()) -> None:
